describe('XMarker', function () {
	describe("#_size", function () {
		...
		describe("when a XMarker is added to the map ", function () {
      it("should have center and size set", function () {
        var marker = L.esri.Renderers.xMarker([45.50, -122.70], 20).addTo(map);
        ...
        expect(marker.getLatLng().lng).to.eq(-122.70);
			...
			describe("and size is set before adding it", function () {
				it("takes that size", function () {
					var marker = L.esri.Renderers.xMarker([0, 0], 20);
					marker.setSize(15);
					marker.addTo(map);
